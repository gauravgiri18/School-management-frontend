...
import { feesData } from '../data/dummyData';
...
const Fees = () => {
  ...
  const filteredFees = feesData.filter(fee => {
    ...
    return fee.status.toLowerCase() === filterStatus;
  });

  const totalAmount = feesData.reduce((sum, fee) => sum + fee.amount, 0);
  const paidAmount = feesData.filter(fee => fee.status === 'Submitted').reduce((sum, fee) => sum + fee.amount, 0);
  const pendingAmount = totalAmount - paidAmount;
  const paidCount = feesData.filter(fee => fee.status === 'Submitted').length;
  const pendingCount = feesData.filter(fee => fee.status === 'Pending').length;
    ...
    <div className="space-y-6">
      ...
      <div className="card">
        ...
        <p className="text-gray-600 dark:text-gray-400">
          Track your monthly fee payments and view payment history.
        </p>
      </div>
        ...
        <div className="card">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{paidCount}/{feesData.length}</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="card">
          ...
          <div className="space-y-3">
            <div className="flex items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
              ...
              <div>
                <p className="text-sm font-medium text-gray-900 dark:text-white">Online Banking</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">Instant payment processing</p>
              </div>
            </div>
            <div className="flex items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
              ...
              <div>
                <p className="text-sm font-medium text-gray-900 dark:text-white">Mobile Payment</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">UPI, Paytm, PhonePe</p>
              </div>
            </div>
            <div className="flex items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">At school office</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="card">
          ...
          <div className="space-y-3 text-sm text-gray-600 dark:text-gray-400">
            <div className="flex items-start">
              ...
              <p>Fee payment is due by the 5th of each month</p>
            </div>
            <div className="flex items-start">
              <span className="w-2 h-2 bg-primary-600 rounded-full mt-2 mr-3 flex-shrink-0"></span>
            ...
            <div className="flex items-start">
              <span className="w-2 h-2 bg-primary-600 rounded-full mt-2 mr-3 flex-shrink-0"></span>
              <p>Payment receipts will be generated automatically</p>
            </div>
            <div className="flex items-start">
              <span className="w-2 h-2 bg-primary-600 rounded-full mt-2 mr-3 flex-shrink-0"></span>
              <p>Contact the accounts office for any payment issues</p>
            </div>
          </div>
        </div>
      </div>
